...
<details>
  ...
  &nbsp;&nbsp;멀티 스레드는 하나의 프로세스에서 스택 이외의 메모리 공간을 공유하는 만큼 문맥 교환을 위한 비용이 적습니다. 다만 메모리를 공유하는 만큼 오류가 발생하면 다른 스레드에 영향을 줄 수 있습니다. 추가로 스레드가 독립적인 스택 영역을 할당 받는 이유는 독립적으로 함수 호출이 가능한 만큼 
</details>
