...
>
>&nbsp;&nbsp;javascript의 파싱이 끝나고 javascript 코드가 실행되는 과정에서 HTML 블로킹이 발생하기 때문에 등장한다면 아직 DOM에 추가되어 있지 않은 요소에 접근하는 것이므로 이 코드는 의도대로 동작하지 않을 것입니다. 띠리서 DOM에 접근하는 코드가 있는 javascript라면 HTML문서 하단에 `script` 태그를 추가하는 편이 오류를 방지할 수 있고, 중간에 DOM 생성을 블로킹하지 않으므로 초기 렌더링 성능에도 좋습니다.
