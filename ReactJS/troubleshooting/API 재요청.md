...
&nbsp;&nbsp;이번 포스트에서는 다양한 지연시간 별 API 재요청 함수를 작성해보았습니다. 재요청 로직은 일시적인 서비스 장애로부터 사용자를 보호해 서비스의 안정성을 높이므로 적절히 활용하는 것이 좋을 것 같습니다. `npm`에는 재요청 로직을 처리할 수 있는 다양한 라이브러리가 제공되고 있습니다. 저는 주로 API 요청 처리에 `axios`를 활용하는 만큼 이후 프로젝트에서 필요하다면 [axios-retry](https://www.npmjs.com/package/axios-retry)를 사용해보며 이번 포스트에서 작성한 재요청 로직과 비교해보는 시간을 
