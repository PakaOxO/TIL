...
&nbsp;&nbsp;웹 서비스를 개발하면서 서비스의 성능은 서버의 부하를 줄여줄 뿐만 아니라 서비스를 이용하는 사용자 경험과 직결되어 있는 요소입니다. 개발하는 서비스의 규모가 증가함에 따라 최적화를 위해 `Lazy loading`을 사용하게 되었고, 사용하기에 앞서 간단하게 내용을 정리해보고자 합니다. 만약 이 글을 읽기에 앞서 React에서 Fetch를 처리하는 방식과 `Suspense`에 대해 잘 모른다면  [다음](./Fetch-types) 포스트에서 다루고 있으니 한번 읽어 보시는 것도 좋습니다.
